...
def index():
    ...
    upcoming_appointments = api_ctrl.fetch_student_appoinments().filter(and_(Appointment.approval_status == ApprovalStatusChoices.APPROVED, Appointment.attendance_status == AttendanceChoices.PENDING)).all()
    old_appointments = api_ctrl.fetch_student_appoinments().filter(and_( or_(Appointment.approval_status == ApprovalStatusChoices.APPROVED, Appointment.approval_status == ApprovalStatusChoices.DECLINED), Appointment.attendance_status != AttendanceChoices.PENDING)).all()
    pending_appointments = api_ctrl.fetch_student_appoinments().filter(Appointment.approval_status == ApprovalStatusChoices.PENDING).all()
    if tag==None:
